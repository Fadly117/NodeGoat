pipeline {
    ...
    stages {
        ...
        stage('SAST SonarQube') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh 'sonar-scanner -Dsonar.projectKey=nodegoat -Dsonar.qualitygate.wait=true -Dsonar.sources=. -Dsonar.host.url=http://192.168.1.18:9000 -Dsonar.token=sqp_b18d55ccf63ef9597da25e87e9f7f8f291a0131f' 
                }
            }
        }
        ...
        stage('Deploy Docker Image') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "DeploymentSSHKey", keyFileVariable: 'keyfile')]) {
                    ...
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no fadly@192.168.1.18 docker run -it --detach -p 4000:4000 --name nodegoat --network host fadly31/nodegoat'
                }
            }
        }
   }
